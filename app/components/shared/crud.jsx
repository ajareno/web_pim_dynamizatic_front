...
import { getAlumnoExamenes, postAlumnoExamen } from "@/app/api-endpoints/alumno_examen";
...
const Crud = ({ getRegistros, getRegistrosCount, botones, columnas, deleteRegistro, headerCrud, seccion,
    ...
    let _abriendoExamen = false;

    const realizarRegistro = async (registroSeleccionado) => {

        if (_abriendoExamen) return;
        _abriendoExamen = true;

        try {
            // id que viene de la vista
            const posibleId = Number(registroSeleccionado?.id);
            let idToOpen = null;

            // Comprueba la existencia de un alumno_examen con ese id
            if (Number.isFinite(posibleId)) {
                const existente = (await getAlumnoExamenes(JSON.stringify({
                    where: { id: posibleId }, fields: { id: true }, limit: 1
                })))?.[0];
                if (existente?.id) idToOpen = Number(existente.id);
            }

            // Si no existe, intenta recuperar alumnoId, examenId y crea el intento
            if (!idToOpen) {
                const alumnoId = Number(registroSeleccionado?.alumnoId ?? registroSeleccionado?.alumno_id);
                const examenId = Number(registroSeleccionado?.examenId ?? registroSeleccionado?.examen_id);

                if (!Number.isFinite(alumnoId) || !Number.isFinite(examenId)) {
                    toast.current?.show({
                        severity: "error",
                        summary: intl.formatMessage({ id: "Error" }),
                        detail: "Faltan alumnoId o examenId en la fila seleccionada.",
                    });
                    return;
                }

                const usu = Number(getUsuarioSesion()?.id);
                const nuevo = await postAlumnoExamen({ alumnoId, examenId, usuCreacion: usu });
                idToOpen = Number(nuevo.id);
            }

            // CAMBIO: Guardar en sessionStorage en lugar de URL
            sessionStorage.setItem("exam_ret_url", window.location.href);
            sessionStorage.setItem("registroSeleccionado", JSON.stringify(registroSeleccionado));
            sessionStorage.setItem("examTokenId", idToOpen.toString());
            
            // Navegar sin token en la URL
            router.push('/alumno/examenes');
            ...
            toast.current?.show({
                severity: "error",
                summary: intl.formatMessage({ id: "Error" }),
                detail: intl.formatMessage({ id: "NoSePudoAbrirExamen", defaultMessage: "No se pudo abrir/crear el intento." }),
            });
        } finally {
            _abriendoExamen = false;
        }
    };
