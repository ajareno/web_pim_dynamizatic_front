...
import { postPlanificadorCategoria, patchPlanificadorCategoria } from "@/app/api-endpoints/planificador_categoria";
import 'primeicons/primeicons.css';
import { getUsuarioSesion } from "@/app/utility/Utils";
import EditarDatosPlanificadorCategoria from "./EditarDatosPlanificadorCategoria";
import { useIntl } from 'react-intl';

const EditarPlanificadorCategoria = ({ idEditar, setIdEditar, rowData, emptyRegistro, setRegistroResult, listaTipoArchivos, seccion, editable }) => {
    const intl = useIntl();
    ...
    const validaciones = async () => {
        ...
        const validaNombre = planificadorCategoria.nombre === undefined || planificadorCategoria.nombre === "";
        // const validaEmpresa = empresaSeleccionada == null || empresaSeleccionada.id === "";
        // const validaDescripcion = planificadorCategoria.descripcion === undefined || planificadorCategoria.descripcion === "";
        //
        //Si existe algún bloque vacio entonces no se puede guardar
        //
        return !validaNombre //(!validaEmpresa && !validaNombre && !validaDescripcion)
    }

    const guardarIncidenciaTipo = async () => {
        setEstadoGuardando(true);
        setEstadoGuardandoBoton(true);
        if (await validaciones()) {
            // Obtenemos el registro actual y solo entramos si tiene nombre y contenido
            let objGuardar = { ...planificadorCategoria };
            ...
            <div className="grid Empresa">
                <div className="col-12">
                    <div className="card">
                        ...
                        <h2>{header} {(intl.formatMessage({ id: 'Categoría del Planificador' })).toLowerCase()}</h2>
